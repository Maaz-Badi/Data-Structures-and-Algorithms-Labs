...
def put(hashtable, key, data, size):
    ...
    p = hash_function(key,size)
    tombstone = []
    if hashtable[0][p] == None:
        hashtable[0][p],hashtable[1][p] = key, data
    else:
        if hashtable[0][p] == key:
            hashtable[1][p] = data
        else:
            newhash = p
            iteration = 1
            while hashtable[0][newhash] is not None and hashtable[0][newhash] != key:
                ...
                newhash = collision_resolver(key, size, iteration)
                if newhash == p:
                    break
                iteration +=1
            ...
            elif len(tombstone) != 0:
                    hashtable[0][tombstone[0]], hashtable[1][tombstone[0]] = key,data
            elif hashtable[0][newhash] is None:
                hashtable[0][newhash], hashtable[1][newhash] = key,data
...
def get(hashtable, key, size):
    ...
    while hashtable[0][pos] is not None:
        if key == hashtable[0][pos] and hashtable[1][pos] is not None:
            return hashtable[1][pos]
        else:
            pos = collision_resolver(key,size,iteration)
            if pos == start:
                break
            iteration +=1
    return None
