def partition_high(lst,low,high,column):
    pivot = low 
    i= low + 1
...
def partition_mid_high(lst,low,high,column):
    pivot = high
    lst[low], lst[pivot] = lst[pivot],lst[low]
    return partition_high(lst,low,high,column)
...
def quick_sort_by_column_number(matrix, low, high, column):
    ...
    if low<high:
        pi = partition_mid_high(matrix, low, high,column)
        print(matrix)
